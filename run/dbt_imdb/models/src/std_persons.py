...
def source(*args, dbt_load_df_function):
    sources = {"std.name.basics": "read_csv_auto(\u0027data/name.basics.tsv.gz\u0027, sample_size=-1, quote=\"\\t\")"}
    key = '.'.join(args)
...
class this:
    ...
    database = "imdb"
    schema = "main"
    identifier = "std_persons"
    
    def __repr__(self):
        return '"imdb"."main"."std_persons"'
...
def materialize(df, con):
    ...
    con.execute('create table "imdb"."main"."std_persons__dbt_tmp" as select * from df')
